...
public class MainPanel {
    ...
    private static SerialPort serialPort02 = null; // 保存串口2对象
    private static SerialPort serialPort03 = null; // 保存串口3对象
    private static SerialPort serialPort04 = null; // 保存串口4对象
                        ...
                        while (inputStream.available() > 0) {
                            ...
                            if (65 == newData) { // 'A'
                                ...
                                if (alltrailData.containsKey(SerialTemp.substring(0, 1))) {
                                    ...
                                    double distanceOne = getDistance(Double.parseDouble(ENH_array[1]), Double.parseDouble(ENH_array[0]), Double.parseDouble(allTrail[allTrail.length - 3]), Double.parseDouble(allTrail[allTrail.length - 4]));
                                    ...
                                    double distance = getDistance(Double.parseDouble(ENH_array[1]), Double.parseDouble(ENH_array[0]), Double.parseDouble(allTrail[3]), Double.parseDouble(allTrail[2]));
                                    ...
                                    if (num < 1 && distance > 40) {
                                        ...
                                        String st6 = "";
                                        for (int i = 0; i < allTrail.length; i++) {
                                            if (i == allTrail.length - 1) {
                                                st6 += allTrail[i];
                                            } else {
                                                st6 += allTrail[i] + ",";
                                            }
                                        }
                                        alltrailData.put(SerialTemp.substring(0, 1), st6);//num>1表示已离开原点
                                    }
                                    //计算用时
                                    if (num > 0 && distance < 40) {
                                        ...
                                        alltrailData.put(SerialTemp.substring(0, 1), st7);//再次回到第一个轨迹点附近时，num-1
